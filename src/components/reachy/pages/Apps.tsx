    ...
    >
      ...
      <Box sx={{ px: 2.5, py: 2.5, display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        >
          ...
          {emoji && (
            <Typography component="span" sx={{ fontSize: 28, lineHeight: 1, flexShrink: 0 }}>
              {emoji}
            </Typography>
          )}
        </Box>
        ...
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          {formattedDate ? (
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              ...
              <Typography sx={{ fontSize: 11, fontWeight: 500, color: "#999" }}>
                {formattedDate}
              </Typography>
            </Box>
          ) : (
...
export default function AppsPage() {
  ...
  // Build the Fuse index on the main thread (JS-only catalog is small).
  const fuse = useMemo(() => {
    const enriched: EnrichedApp[] = apps.map((app) => ({
      id: app.id,
      name: app.name,
      ...
      const results = fuse.search(query).map((r) => {
        ...
        if (r.matches) {
          for (const m of r.matches) {
            if (m.key) matches[m.key] = m.indices as unknown as [number, number][];
          }
        }
        return { id: (r.item as EnrichedApp).id, score: r.score, matches };
      });
  ...
  const filteredApps = useMemo(() => {
    ...
    if (searchResults !== null) {
      const scoreMap = new Map(searchResults.map((r) => [r.id, r.score ?? 1]));
  ...
  const matchDataMap = useMemo(() => {
    ...
    const map = new Map<string, MatchMap>();
    for (const r of searchResults) map.set(r.id, r.matches);
    return map;
  }, [searchResults]);
  ...
  return (
    <>
          ...
          >
            ...
            <Box sx={{ textAlign: { xs: "center", md: "left" } }}>
              ...
              >
                <Typography
                  variant="caption"
                  sx={{ color: "rgba(255,255,255,0.7)", fontWeight: 500, fontSize: 11 }}
                >
                  Powered by
                </Typography>
              ...
              <Typography
                variant="h6"
                sx={{ color: "rgba(255,255,255,0.7)", fontWeight: 400, maxWidth: 550, lineHeight: 1.7 }}
              >
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                size="small"
                sx={{ color: "#999", "&.Mui-checked": { color: "#FF9500" } }}
              />
            }
            label={
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, flexWrap: "wrap" }}>
            ...
            <Chip
              ...
              sx={{
                ...
                bgcolor: !selectedCategory ? "rgba(255, 149, 0, 0.12)" : "rgba(0, 0, 0, 0.04)",
                ...
                "&:hover": {
                  bgcolor: !selectedCategory ? "rgba(255, 149, 0, 0.18)" : "rgba(0, 0, 0, 0.08)",
                },
              ...
              return (
                <Chip
                  ...
                  sx={{
                    ...
                    bgcolor: isSelected ? "rgba(255, 149, 0, 0.12)" : "rgba(0, 0, 0, 0.04)",
                    ...
                    transition: "all 0.15s ease",
                    "&:hover": {
                      bgcolor: isSelected ? "rgba(255, 149, 0, 0.18)" : "rgba(0, 0, 0, 0.08)",
                    },
                    "& .MuiChip-label": { px: 1.5 },
                  }}
                />
      ...
      <Container maxWidth="lg" sx={{ pb: 10 }}>
        {loading ? (
          ...
          <Box sx={{ textAlign: "center", py: 10 }}>
            ...
            <Typography sx={{ color: "#999" }}>Try adjusting your search or filters</Typography>
          </Box>
        ) : (
          <Box
            sx={{
      ...
      <InstallModal open={installModalOpen} onClose={handleCloseInstallModal} app={selectedApp} />
    </>
  );
}
